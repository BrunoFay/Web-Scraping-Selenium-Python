...
airforce_selector=browser.find_element(By.XPATH,'/html/body/div[2]/div[3]/div[2]/div/nav/ul/li[4]/div/div/div/div/div/div[3]/div/a')
airforce_selector.click()
...
def handle_scroll_page():
    ...
    while True:
        ...
        current_scrool_height = browser.execute_script("return document.body.scrollHeight")
        positions.append(current_scrool_height)
        if (repeat_positions[current_scrool_height] >= 10):
         break
...
def create_product_dict(list):
  ...
  for tenis_card in tenis_cards:
    tenis={}
    imgs_container=tenis_card.find('a',attrs={'itemprop':'url'})
    print(imgs_container)
    ...
    list.append(tenis) """
create_product_dict(nike_airforce_list)
...
print(len(nike_airmax90_list))
print(len(nike_airmax97_list))
print(len(nike_airforce_list))
